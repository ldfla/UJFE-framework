...
final class ElementRenderingTest {
    ...
    void rendersComponentNodeChildren() {
        ...
        assertEquals("<div><p>Component</p></div>", div().child(component(sample))
            .render());
    }
    ...
    void rendersCoreNodeChildVarargs() {
        ...
        assertEquals("<div><p>Component</p><p>Tail</p></div>", div().children(nodes)
            .render());
    }
    ...
    void rendersFormElementsAndInputTypes() {
        String html = form()
            ...
            .child(select()
                .name("role")
                .child(option("Admin").value("admin")
                    .selected(true))
                .child(option("User").value("user")))
            .child(textarea("Observacao").name("notes")
                .rows(3)
                .cols(20))
            .render();
    ...
    void rendersAdditionalFormElements() {
        String html = fieldset()
            ...
            .child(inputRange().name("volume")
                .min("0")
                .max("10"))
            .child(inputButton().value("Validar"))
            .child(inputImage().attr("src", "/submit.png")
                .attr("alt", "Enviar"))
            .child(inputSubmit().value("Enviar"))
            .child(inputReset().value("Limpar"))
            .child(datalist().id("cities")
                .child(option().value("Sao Paulo")))
            .child(output("42").name("result"))
            .child(progress().value("3")
                .max("10"))
            .child(meter().value("0.7")
                .min("0")
                .max("1"))
            .render();
    ...
    void rendersGlobalAttributesAndMediaElements() {
        String html = div()
            ...
            .translate(false)
            .hidden(false)
            .child(canvas().width(320)
                .height(180)
                .ariaLabel("Canvas demo"))
            .child(video().src("/movie.mp4")
                .poster("/poster.png")
                .controls(true)
                .playsInline(true))
            .child(audio().controls(true)
                .child(source().src("https://cdn.example.test/audio.mp3")
                    .type("audio/mpeg")))
            .child(picture().child(source().src("/image.webp")
                    .attr("media", "(min-width: 800px)"))
                .child(img().src("/image.png")
                    .alt("Imagem")))
            .render();
    ...
    void rendersExtendedFormAttributes() {
        ...
        selected.set(0);
        assertTrue(form.render()
            .contains("<optgroup label=\"Languages\"><option>Java</option></optgroup>"));
    }
    ...
    void rendersExtendedMediaAttributes() {
        assertEquals("<video src=\"/movie.mp4\" crossorigin=\"anonymous\" referrerpolicy=\"no-referrer\" autoplay loop muted></video>",
            video()
                ...
                .muted(true)
                .render());
        assertEquals("<img src=\"/logo.png\" loading=\"lazy\">", img().src("/logo.png")
            .loading("lazy")
            .render());
    }
    ...
    void rendersCustomElementWithHyphen() {
        ...
        assertEquals("my-card", Element.custom("my-card")
            .tagName());
    ...
    void rendersSvgTagsThroughControlledNamespace() {
        ...
        assertEquals(ElementNamespace.SVG, Element.svg("path")
            .namespace());
    ...
    void rendersMathMlTagsThroughControlledNamespace() {
        ...
        assertEquals(ElementNamespace.MATHML, Element.mathMl("mi")
            .namespace());
    ...
    void helpersRemainOptionalSugarOverGenericElements() {
        assertEquals(Element.of("div")
            .tagName(), div().tagName());
        assertEquals(ElementNamespace.HTML, div().namespace());
        assertEquals(Element.of("svg")
            .tagName(), svg().tagName());
        assertEquals(ElementNamespace.SVG, svg().namespace());
        assertEquals(ElementNamespace.MATHML, math().namespace());
    }
    ...
    void rendersGenericAndModernHtmlElements() {
        String html = section()
            ...
            .child(table()
                ...
                .child(thead().child(tr().child(th("Name"))
                    .child(th("Value"))))
                .child(tbody().child(tr().child(td("Users"))
                    .child(td("42"))))
                .child(tfoot().child(tr().child(td("Total"))
                    .child(td("42")))))
            .child(figure()
                .child(svg().attr("viewBox", "0 0 10 10")
                    .child(element("path").attr("d", "M0 0h10v10H0z")))
                .child(figcaption("Vector")))
            .child(div().popover(true)
                .child("Popover"))
            .render();
