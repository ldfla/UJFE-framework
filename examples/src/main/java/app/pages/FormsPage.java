...
public final class FormsPage implements Component {
    ...
    public Node render() {
        return main()
                .css("min-h-screen bg-slate-50 text-slate-900 p-8 flex flex-col gap-6")
                .child(a("Back to home").href("/").css("text-sm font-semibold text-primary-700"))
                .child(section()
                        .css("max-w-3xl rounded-lg border border-slate-200 bg-white p-6 shadow-sm flex flex-col gap-5")
                        .child(h1("Live form events").css("text-3xl font-bold"))
                        ...
                        .child(form()
                                ...
                                .child(field("Profile", "forms-profile", select()
                                        ...
                                        .css(inputClass())
                                        .child(option("Java backend").value("backend").selected(() -> "backend".equals(profile.get())))
                                        .child(option("Full stack Java").value("fullstack").selected(() -> "fullstack".equals(profile.get())))))
                                .child(field("Interests", "forms-interests", select()
                                        .name("interests")
                                        .multiple(true)
                                        .onChange(interests::set)
                                        .css(inputClass() + " h-24 py-2")
                                        .child(option("Java").value("java").selected(() -> selected(interests.get(), "java")))
                                        .child(option("Security").value("security").selected(() -> selected(interests.get(), "security")))
                                        .child(option("Runtime").value("runtime").selected(() -> selected(interests.get(), "runtime")))))
                                .child(label()
                                ...
                                .child(div()
                                        .css("flex flex-wrap gap-4 text-sm text-slate-700")
                                        .child(label()
                        ...
                        .child(div()
                                .css("rounded bg-slate-100 border border-slate-200 p-4 text-sm text-slate-700 flex flex-col gap-2")
    ...
    private static Node field(String labelText, String id, Element control) {
        return div()
    ...
    private static String inputClass() {
        return "h-10 rounded border border-slate-300 px-3 text-sm";
    }
